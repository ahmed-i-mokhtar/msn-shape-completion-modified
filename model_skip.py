...
class SoftPool1D(nn.Module):
    ...
    def forward(self, x):
        ...
        weights = F.softmax(x_unf, dim=3)
        
        # Compute weighted sum
        weighted_sum = (weights * x_unf).sum(dim=3)
...
class PointNetfeat(nn.Module):
    def __init__(self, num_points = 8192, global_feat = True):
        ...
        self.soft_pool = SoftPool1D(1024, 1)
    ...
    def forward(self, x):
        ...
        x1_max,_ = torch.max(x1, 2) # B x 256
        x2_max,_ = torch.max(x2, 2) # B x 512
        #Modification B (Adding SoftPool instead of max pooling)
        x3_max,_ = torch.max(x3, 2) # B x 1024
        ...
        x1 = x1_max.view(-1, 256) #reshape tensor to 1D tensor B x 256
        x2 = x2_max.view(-1, 512) #reshape tensor to 1D tensor B x 512
        x = x3_max.view(-1, 1024) #reshape tensor to 1D tensor B x 1024
        return x, x2, x1
